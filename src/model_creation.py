...
class RandomForestModel:
    ...
    def train(self, X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series)-> None:
        ...
        search = RandomizedSearchCV(
            estimator=RandomForestClassifier(bootstrap= True, oob_score= True, class_weight= "balanced", random_state= 42, n_jobs= 1),
            param_distributions={"n_estimators": [200, 300, 500, 700, 1000], "max_depth": [None, 8, 12, 16, 20, 30], "min_samples_split": [2, 5, 10, 20], "min_samples_leaf": [1, 2, 4, 8, 16], "max_features": ["sqrt", "log2", 0.5, None], "max_samples": [None, 0.6, 0.8]},
        ...
        logger.info(f"OOB Score: {self._model.oob_score_:.4f}")
